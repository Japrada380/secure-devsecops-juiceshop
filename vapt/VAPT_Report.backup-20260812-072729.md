...
## 8.4 V-09 – Referencia Directa Insegura a Objetos (IDOR)
...
Durante la evaluación se identificó que el endpoint del carrito permite acceder a recursos utilizando únicamente el identificador del carrito (`BasketId`), sin verificar correctamente que dicho recurso pertenezca al usuario autenticado.
...
El servidor respondió con información correspondiente al:

```json
"UserId": 1
```

permitiendo visualizar información de un recurso perteneciente a otro usuario.
...
- Burp Suite Repeater.
- Respuesta HTTP 200 OK.
- Captura del Repeater mostrando la modificación del identificador.
- Archivo:

```
vapt/evidence/burp/V09_IDOR_Basket.txt
```
...
Un atacante autenticado puede consultar información perteneciente a otros usuarios.

**Integridad:** Baja.

Si existieran endpoints de modificación asociados, podría alterarse información de terceros.

**Disponibilidad:** Baja.

No afecta directamente la disponibilidad.

**Ley 1581 de 2012**

Existe riesgo de acceso no autorizado a datos personales de otros usuarios.

---

### Remediación

Se recomienda validar siempre que el recurso solicitado pertenezca al usuario autenticado antes de devolver la información.
...
Después de implementar la validación, cualquier intento de acceder a un recurso de otro usuario debe responder:
...
**No reproducible en la versión evaluada de OWASP Juice Shop.**

---

## Objetivo

Verificar si la aplicación aceptaba atributos adicionales enviados por el cliente que permitieran modificar propiedades sensibles del modelo (por ejemplo: `role`, `isAdmin`, `UserId`, etc.).

---

## Prueba realizada

Se identificó la API B2B documentada mediante Swagger:

```
POST /b2b/v2/orders
```

Se modificó el cuerpo de la petición agregando un atributo no documentado:
...
La solicitud nunca alcanzó la lógica de negocio debido a que la API utiliza un mecanismo de autenticación diferente al JWT obtenido mediante el login estándar de Juice Shop.

Por esta razón no fue posible validar si existía una vulnerabilidad de Mass Assignment.

---

## Conclusión

Durante las pruebas realizadas **no se obtuvo evidencia suficiente** para confirmar la existencia de una vulnerabilidad CWE-915 en la versión evaluada de OWASP Juice Shop.

Se recomienda realizar una nueva validación utilizando credenciales válidas para la API B2B o revisando directamente el código fuente del controlador correspondiente.

# Hallazgo adicional – Cross-Site Scripting (XSS)
...
Se ejecutó código JavaScript en el navegador mostrando una ventana emergente con el mensaje:
...
Un atacante puede ejecutar código JavaScript en el navegador de la víctima.
...
Escapar correctamente la entrada del usuario.

Aplicar sanitización utilizando librerías como DOMPurify.
...
Después de aplicar la corrección, el payload debe mostrarse como texto plano y no ejecutar JavaScript.

## Prueba adicional – Stored XSS

Se intentó reproducir una vulnerabilidad Stored XSS mediante el formulario Customer Feedback utilizando el payload:

```html
<img src=x onerror=alert('XSS Feedback')>